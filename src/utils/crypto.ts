// SHA-256 implementation (no crypto library)
function sha256(message: string): string {
    ...
    const paddedLength = Math.ceil((bits + 65) / 512) * 512;
    ...
    for (let i = 0; i < 8; i++) {
        buffer[buffer.length - 8 + i] = (bits / 2 ** (56 - i * 8)) & 0xff;
    }
...
export { base64UrlEncode, hexToBinary, sha256 };
