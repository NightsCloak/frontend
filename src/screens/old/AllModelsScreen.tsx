...
const AllModelsScreen: FC = () => {
    const { classes } = useStyles();
    const [assets, setAssets] = useState<Asset[] | null>(null);
    const [getModels, getModelsState] = useGetAssetsAdminMutation();
    // const assetTags = useGetAssetTagsQuery(null);
    const { updateBreadcrumbs, updateTabTitle } = useTools();

    const { nav, queryOptions, handleTagChipClicked } = useMeta(getModels, getModelsState, {
        include: 'project',
        // tags: assetTags,
        sortSize: true,
    });
    ...
    useEffect(() => {
        updateBreadcrumbs([{ name: 'Admin', uri: '/admin' }, { name: 'Models' }]);
...
export default AllModelsScreen;
