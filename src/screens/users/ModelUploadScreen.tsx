...
const ModelUploadScreen: FC = () => {
    ...
    const [asset, setAsset] = useState<UserAsset | undefined>(undefined);
    const [errorMessage, setErrorMessage] = useState<string | undefined>(undefined);
    const [getUserAsset, { data, error }] = useGetUserAssetMutation();
    ...
    useEffect(() => {
        updateBreadcrumbs([{ name: 'Model Upload' }]);
        updateTabTitle('Model Upload');
    }, []);
    ...
    useEffect(() => {
        if (data) {
            if (!data.is_pending) {
                navigate(`/home/models/${data.id}/editor`);
                return;
            }
