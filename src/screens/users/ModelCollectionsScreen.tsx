...
import { useGetUserAssetCollectionsMutation } from '@intractinc/base/redux/features/userAssetCollection';
import AddCollection from '@intractinc/base/components/UserAssetCollections/AddCollection';
...
const ModelCollectionsScreen = () => {
    ...
    const [getCollections, getCollectionsState] = useGetUserAssetCollectionsMutation();
    ...
    const { nav, queryOptions } = useMeta(getCollections, getCollectionsState, { include: 'assetsCount' });
    ...
    useEffect(() => {
        updateTools([<AddCollection key={'add_collection'} />]);
        updateBreadcrumbs([{ name: 'Folders' }]);
        updateTabTitle('Folders');
    }, []);
                    ...
                    collections.length ? (
                        collections.map((collection) => (
                            <Grid key={`collection_${collection.id}`} item xs={15} sm={5} lg={3} p={2}>
                                <UserAssetCollectionCard {...{ collection, refetch: refetchCollections }} />
                            </Grid>
                        ))
                    ) : (
                        <Grid item xs={15}>
