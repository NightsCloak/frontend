...
import { useGetTexturesAdminMutation } from '@intractinc/base/redux/features/adminTexture';
import TextureCard from '@intractinc/base/components/Textures/TextureCard';
import StoreTexture from '@intractinc/base/components/Textures/StoreTexture';

const ImagesScreen = () => {
    const { classes } = useStyles();
    const [getTextures, getTexturesState] = useGetTexturesAdminMutation();
    ...
    useEffect(() => {
        updateBreadcrumbs([{ name: 'Admin', uri: '/admin' }, { name: 'Public Images' }]);
        updateTabTitle('Public Images');
        updateTools([<StoreTexture key={'store_texture'} {...{ onSuccess: refetchTextures }} />]);
    }, [queryOptions]);
