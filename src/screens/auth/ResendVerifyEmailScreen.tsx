...
const ResendVerifyEmailScreen: FC = () => {
    ...
    const [submit, { isLoading, isSuccess, error }] = useResendEmailVerificationMutation();
    ...
    useEffect(() => {
        if (error) {
            ...
            const data = response as ResendEmailVerificationResponse;
            if (data.message) {
    ...
    useEffect(() => {
        updateTabTitle('Resend Account Verification');
    }, []);
            ...
            <div className={classes.root}>
                <Paper className={classes.formRoot}>
                    ...
                    <Typography style={{ margin: theme.spacing(3) }} variant={'body1'}>
                        Please check your email for instructions on completing your account verification.
                    </Typography>
                    <Button
                        fullWidth={true}
                        onClick={() => navigate('/')}
    ...
    return (
        <div className={classes.root}>
            <Paper className={classes.formRoot}>
                <div className={classes.header}>
                    <Typography variant={'h4'}>
                        <Email style={{ fontSize: 25 }} /> Resend Verification Email
                    </Typography>
                </div>
                {isLoading ? (
                    ...
                    <>
                        <Typography style={{ margin: theme.spacing(3) }} variant={'subtitle1'}>
                            Enter your account email below to resend the verification email.
                        </Typography>
